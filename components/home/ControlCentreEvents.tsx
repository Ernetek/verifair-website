...
const trendValues = publicDemonstrationScenario.observations
  .filter((observation) => observation.monitorId === "WORK_ZONE_A" && observation.metricId === "RESPIRABLE_DUST" && observation.reading.status === "available")
  .map((observation) => observation.reading.status === "available" ? observation.reading.value : 0);
  ...
  return (
    <section className="bg-slate-100" aria-labelledby="events-view-heading">
      <header className="border-b border-slate-200 bg-white px-4 py-4 sm:px-5">
        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-red-700">Control Centre page</p>
          ...
          <div className="min-w-0 p-3 sm:p-5">
            <div className="border border-slate-300 bg-white">
              <div className="border-b border-slate-200 p-4 sm:p-5">
                <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-[10px] font-black text-red-700">{INCIDENT_ID}</p><h4 className="mt-1 text-xl font-black text-slate-950 sm:text-2xl">Respirable Dust action condition</h4><p className="mt-1 text-sm text-slate-600">Zone A · Monitoring Location 1</p></div><span className="border border-red-200 bg-red-50 px-3 py-2 text-xs font-black uppercase text-red-800">{workflowLabel(snapshot)}</span></div>
                {!incident.investigationStarted && !incident.closed && <button type="button" onClick={acknowledgeAndStart} className="mt-4 min-h-11 bg-red-700 px-4 text-xs font-black text-white">ACKNOWLEDGE, ASSIGN &amp; START WORK</button>}
              </div>
              ...
              <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
                ...
                <div>
                  ...
                  <div className="mt-2 border border-slate-200 bg-slate-50 p-3"><div className="flex items-end justify-between gap-3"><p className="text-sm font-black text-slate-950">Respirable Dust</p><p className="font-mono text-xl font-black text-slate-950">{currentValue(snapshot)} <span className="text-[10px] text-slate-500">{PARTICULATE_UNIT}</span></p></div><svg viewBox="0 0 620 190" className="mt-3 h-40 w-full" role="img" aria-label="Respirable Dust event trend">{[40, 80, 120, 160].map((y) => <line key={y} x1="20" x2="600" y1={y} y2={y} stroke="#cbd5e1" />)}<polyline points={trendPoints} fill="none" stroke="#0369a1" strokeWidth="5" strokeLinejoin="round" /><circle cx={20 + (trendValues.length - 1) * (580 / Math.max(trendValues.length - 1, 1))} cy={170 - ((trendValues.at(-1) ?? 0) / maxTrend) * 130} r="7" fill="#0369a1" className="motion-safe:animate-pulse" /></svg></div>
